...
<body
    ...
    <div class="w-full h-screen flex flex-col lg:flex-row">
        ...
        <!-- Sidebar -->
        <div id="sidebar"
            class="hidden lg:flex w-full lg:w-1/4 bg-gradient-to-b from-blue-100 to-gray-500 p-4 lg:p-6 m-0 lg:m-4 rounded-none lg:rounded-2xl shadow-none lg:shadow-custom flex-col items-center fixed lg:static top-0 left-0 h-[100vh] lg:h-[100vh] max-h-[95vh] z-40 transition-transform duration-300 ease-in-out transform lg:transform-none bg-opacity-95 overflow-y-auto">
            ...
            <ul class="space-y-3 w-full">
                <li
                    class="w-full py-3 flex items-center text-black font-semibold cursor-pointer hover:bg-gray-200 rounded-lg p-3 transition-colors duration-200 hover:scale-105">
                ...
                <li
                    class="w-full py-3 flex items-center text-black font-semibold cursor-pointer bg-gray-200 rounded-lg p-3 transition-colors duration-200 hover:scale-105">
                ...
                <li
                    ...
                    <a href="{{ route('leave.create') }}" class="flex items-center w-full">
                        <img src="{{ asset('storage/photos/folder.png') }}" class="w-8 h-8 mr-3"
...
=======
                    ...
                    <a href="{{ url('') }}" class="flex items-center w-full">
                        <img src="{{ asset('storage/photos/chat-box.png') }}" class="w-8 h-8 mr-3"
                            alt="Notifications" />
                        Write Notification
...
>>>>>>> 6e4e6c85530809b26e0e950ca320b3252e81058d
                ...
                <li
                    class="mt-8 w-full py-3 flex items-center text-red-500 font-bold cursor-pointer hover:bg-gray-300 rounded-lg p-3 transition-colors duration-200 hover:scale-105">
        ...
        <div class="w-full lg:w-3/4 p-4 lg:p-8 relative">
            ...
            @if (session('success'))
            <div
                class="bg-green-100 text-green-700 p-4 rounded-2xl mb-6 shadow-custom transition-all duration-300 hover:shadow-custom-hover">
                {{ session('success') }}
            </div>
            @endif
            @if (session('error'))
            <div
                class="bg-red-100 text-red-500 p-4 rounded-2xl mb-6 shadow-custom transition-all duration-300 hover:shadow-custom-hover">
                {{ session('error') }}
            </div>
            @endif
                        ...
                        <tr class="border-b hover:bg-blue-50 transition-all duration-300">
                            ...
                            <td class="p-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <a href="{{ route('leave.attachment', [$application->id, 1]) }}" target="_blank"
                                        class="flex items-center text-blue-600 hover:text-blue-800 hover:underline transition-all duration-300">
                            ...
                            <td class="p-4">
                                <form action="{{ route('principal.leave.updateStatus', $application->id) }}"
                                    ...
                                    <button type="submit"
                                        class="bg-green-400 text-gray-800 px-4 py-2 rounded-full hover:bg-green-300 shadow-md hover:shadow-custom-hover transition-all duration-300 hover:scale-105">Approve</button>
                                </form>
                                <form action="{{ route('principal.leave.updateStatus', $application->id) }}"
                                    ...
                                    <textarea name="comment" placeholder="Reason for rejection"
                                        class="border border-gray-300 p-2 mt-1 w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 shadow-sm bg-white transition-all duration-300"
                                        rows="2"></textarea>
                                    @error('comment')
                                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                    @enderror
                                    <button type="submit"
                                        class="bg-red-500 text-gray-800 px-4 py-2 rounded-full hover:bg-red-300 shadow-md hover:shadow-custom-hover transition-all duration-300 hover:scale-105 mt-1">Reject</button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
