...
<head>
    <meta charset="UTF-8">
    <title>Manage Sectional Heads</title>
...
<body class="bg-gray-300 flex items-center justify-center min-h-screen">
    <div class="bg-white shadow-lg rounded-lg w-full h-screen flex">
        ...
        <div class="w-3/4 p-8 overflow-y-auto">
            ...
            <h2 class="text-2xl font-bold mb-6 text-center">Manage Sectional Heads</h2>
            ...
            <table class="min-w-full bg-white border rounded shadow">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-100">
                        <td class="py-2 px-4 border">{{ $sectional->first_name }} {{ $sectional->last_name }}</td>
                        <td class="py-2 px-4 border">{{ $sectional->user_email }}</td>
                        <td class="py-2 px-4 border">{{ $sectional->section ?? 'N/A' }}</td>
                        <td class="py-2 px-4 border">{{ ucfirst($sectional->status) }}</td>
                        <td class="py-2 px-4 border space-x-1">
                            <a href="{{ route('sectionals.edit', $sectional->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-1 px-3 rounded mr-1">Edit</a>
                    ...
                    @empty
                    <tr><td colspan="4" class="text-center py-4">No sectional heads found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
